...
if [ -n "$OVERRIDE_TEXT" ]; then
    ...
    if [ ${#FILES[@]} -eq 0 ]; then
        echo "STATUS:ERROR"
        notify-send "TTS Error" "No audio files were generated."
        exit 1
    fi
    rm -f "$MPV_SOCKET"
    mpv "${FILES[@]}" --no-terminal --input-ipc-server="$MPV_SOCKET" &
    MPV_PID=$!
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if [ -n "$OUTPUT_FILE" ]; then
        ...
        if command -v ffmpeg &>/dev/null; then
            ...
            # Fallback: cat MP3s (best-effort for sequential files)
            cat "${FILES[@]}" > "$OUTPUT_FILE"
        fi
        notify-send "TTS Export" "Saved audio to $OUTPUT_FILE"
    fi

    # Play all segments as a single mpv playlist with IPC for remote control
    mapfile -t FILES < <(find "$WORK_DIR" -maxdepth 1 -name 'seg_*.mp3' | sort)
    if [ ${#FILES[@]} -eq 0 ]; then
        notify-send "TTS Error" "No audio files were generated."
        exit 1
